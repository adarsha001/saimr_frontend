...
export default function Footer() {
  ...
  const trackClick = async (itemType, itemValue) => {
    ...
    try {
      ...
      if (response.ok) {
        console.log(`Click tracked for ${itemType}: ${itemValue}`);
      }
    } catch (error) {
          ...
          <div className="lg:col-span-2">
            ...
            <div className="flex space-x-4">
              {['Facebook', 'Twitter', 'LinkedIn', 'WhatsApp'].map((platform) => (
                <button
                  key={platform}
                  onClick={() => trackClick('social', platform.toLowerCase())}
                  className={`w-10 h-10 bg-gray-800 hover:bg-gray-700 rounded-full flex items-center justify-center transition-all duration-300 ${
                    clickedItem === `social-${platform.toLowerCase()}` ? 'scale-110 bg-white' : ''
                  }`}
                  aria-label={platform}
                >
          ...
          <div>
            ...
            <ul className="space-y-3">
              {['Home', 'About Us', 'Services', 'Projects', 'Contact'].map((link) => (
                <li key={link}>
                  <button
                    onClick={() => trackClick('navigation', link.toLowerCase().replace(' ', '_'))}
          ...
          <div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center space-x-3">
                ...
                <button
                  onClick={() => trackClick('phone', '7788999022')}
                  className={`text-gray-300 hover:text-white transition-all duration-300 ${
                    clickedItem === 'phone-7788999022' ? 'font-semibold scale-105' : ''
                  }`}
                >
                  +91 7788999022
                </button>
              ...
              <div className="flex items-center space-x-3">
                ...
                <button
                  onClick={() => trackClick('email', 'saimrgroups@gmail.com')}
                  className={`text-gray-300 hover:text-white transition-all duration-300 ${
              ...
              <div className="flex items-center space-x-3">
                ...
                <button
                  onClick={() => trackClick('website', 'saimrgroups.com')}
                  className={`text-gray-300 hover:text-white transition-all duration-300 ${
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-4xl mx-auto">
            {[
              ...
              <button
                key={service.name}
                onClick={() => trackClick('service', service.name.toLowerCase().replace(' ', '_'))}
          ...
          <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
            ...
            <div className="flex space-x-6 text-sm">
              {['Privacy Policy', 'Terms of Service', 'Disclaimer'].map((link) => (
                <button
                  key={link}
                  onClick={() => trackClick('legal', link.toLowerCase().replace(' ', '_'))}
